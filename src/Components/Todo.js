...
const Todo = ({ todo, priority, todoObj, onCompleteClick, onEditClick }) => {
    ...
    function handleEditClick(event) {
        ...
        setOpen(false);
        onEditClick(todoObj.id, todoName, todoPriority);
    }
    ...
    return (
        <ListItem sx={{
            ...
            <IconButton color="success" variant="plain" size="sm" onClick={handleCompleteClick}><DoneOutlineTwoToneIcon /></IconButton>
            <IconButton color="warning" variant="plain" size="sm" onClick={_ => setOpen(true)}><ModeTwoToneIcon /></IconButton>

            <Modal open={open} onClose={() => setOpen(false)} disableAutoFocus disableEnforceFocus>
                <ModalDialog
                    sx={{
                        maxWidth: 540,
                        borderRadius: 'md',
                        ...
                        <Stack spacing={2}>
                            <Box>
                                ...
                                >
                                    ...
                                    >
                                        {['Low Priority', 'Medium Priority', 'High Priority'].map((value, index) => (
                                            <div key={index}>
                                                <ListItem>
